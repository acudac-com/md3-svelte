<script lang="ts" module>
	...
	export function Icons(icon: IconType): IconifyIcon {
		return icons[icon];
	}
</script>

<script lang="ts">
	let { class: cls = '', size, icon }: { size?: string; class?: string; icon: IconType } = $props();
</script>

<Icon height={size} width={size} class={cls} icon={icons[icon]}></Icon>
